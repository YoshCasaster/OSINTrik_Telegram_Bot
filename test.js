  ...
  async _poll(apiCallFunction, validationFunction, callName, maxRetries = 10, delay = 1500) {
    ...
    while (retries < maxRetries) {
      ...
      if (retries < maxRetries) {
        console.log(`[POLL RETRY] Retrying ${callName} in ${delay / 1000}s... (Attempt ${retries}/${maxRetries})`);
  ...
  async chat({ prompt, messages, ...rest }) {
    ...
    try {
      ...
      const createChatResponse = await this._axiosInstance.post('/chat', {
        name: "Untittled Session", born_from: "home", kimiplus_id: "kimi",
      ...
      const messagesToSend = (messages && messages.length) ? messages : [{ role: "user", content: prompt }];
      const completionPayload = {
        kimiplus_id: "kimi", model: "kimi", use_search: true, refs: [], history: [],
        scene_labels: [], use_semantic_memory: false, ...rest, messages: messagesToSend,
      };
      ...
      const [streamResponse, chatList, suggestions] = await Promise.all([
        this._axiosInstance.post(`/chat/${chatId}/completion/stream`, completionPayload, { headers: { 'referer': `https://www.kimi.com/chat/${chatId}` }, responseType: 'text' }).catch(e => { console.error("--> Sub-process 'Stream' failed:", e.message); return null; }),
        this.listChats({ offset: 0, size: 5 }, chatId).catch(e => { console.error("--> Sub-process 'ListChats' failed:", e.message); return null; }),
        this.getSuggestions({ query: prompt, scene: "first_round" }).catch(e => { console.error("--> Sub-process 'GetSuggestions' failed:", e.message); return null; })
      ]);
      ...
      if (groupId) {
        ...
        const [historyResult, recommendedResult] = await Promise.all([
          this.getChatHistory(chatId, { last: 10 }).catch(e => { console.error("--> Sub-process 'GetHistory' failed:", e.message); return null; }),
          this._axiosInstance.post('/chat/recommend-prompt', { chat_id: chatId, group_id: groupId, use_search: true }, { responseType: 'text' }).then(res => this.parseStream(res.data)).catch(e => { console.error("--> Sub-process 'GetRecommendations' failed:", e.message); return null; })
        ]);
...
async function main() {
  ...
  try {
    ...
    const comprehensiveResult = await apiClient.chat({
      prompt: "Kapan Hari Kemerdekaan Indonesia?",
    });

    console.log(comprehensiveResult);

  } catch (error) {
    console.error('\n--- ❌❌❌ Proses Utama Gagal Total ❌❌❌ ---');
